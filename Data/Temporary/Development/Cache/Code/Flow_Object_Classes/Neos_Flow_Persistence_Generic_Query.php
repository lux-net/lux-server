 ...
 * @api
 */
class Query_Original implements QueryInterface
{
    /**
     * @var string
     */
 ...
 * $query->matching($query->equals('foo', 'bar'))->setLimit(10)->execute();
 */
class Query extends Query_Original implements \Neos\Flow\ObjectManagement\Proxy\ProxyInterface {
    ...
    public function __construct()
    {
        ...
        if (!array_key_exists(1, $arguments)) $arguments[1] = \Neos\Flow\Core\Bootstrap::$staticObjectManager->get('Neos\Flow\Reflection\ReflectionService');
        ...
        if (!array_key_exists(1, $arguments)) throw new \Neos\Flow\ObjectManagement\Exception\UnresolvedDependenciesException('Missing required constructor argument $reflectionService in class ' . __CLASS__ . '. Note that constructor injection is only support for objects of scope singleton (and this is not a singleton) – for other scopes you must pass each required argument to the constructor yourself.', 1296143788);
        call_user_func_array('parent::__construct', $arguments);
        if ('Neos\Flow\Persistence\Generic\Query' === get_class($this)) {
  ...
  'offset' => 'integer',
    ...
    private function Flow_Proxy_injectProperties()
    {
        ...
        $this->injectQomFactory(new \Neos\Flow\Persistence\Generic\Qom\QueryObjectModelFactory());
...
# PathAndFilename: /var/www/lux/Packages/Framework/Neos.Flow/Classes/Persistence/Generic/Query.php
#